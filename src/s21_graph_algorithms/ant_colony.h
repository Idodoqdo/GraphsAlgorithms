#ifndef SRC_S21_GRAPH_ALGORITHMS_ANT_COLONY_H_
#define SRC_S21_GRAPH_ALGORITHMS_ANT_COLONY_H_
#include <memory>
#include <set>
...
class Ant {
 public:
  Ant(const Graph &graph_distance, std::unique_ptr<Graph> &graph_pheromones,
      std::size_t &index_start)
      : graph_distance_(graph_distance), graph_pheromones_(graph_pheromones) {
  ...
  TsmResult result() { return run_result_; }
...
class Colony {
 public:
  ...
  Colony() = delete;
  //конструторы удалены, поскольку могут возникнуть проблемы при копировании
  //объекта, содержащим unique_ptr
...
#endif  // SRC_S21_GRAPH_ALGORITHMS_ANT_COLONY_H_
